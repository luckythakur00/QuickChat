    ...
    <div className='h-full lg:flex' >
      ...
      <div className='h-[50%] lg:h-full w-full lg:w-1/2 flex flex-col justify-center items-center gap-4 mt-4 sm:my-10 lg:my-20' >
        <div className='flex flex-col justify-center items-center' >
          ...
          <h1 className='text-xl sm:text-2xl font-bold py-1' >Create Account</h1>
        ...
        <form onSubmit={handleCreate} method='post' className='w-[90%] sm:w-full sm:flex flex-col justify-center items-center text-base sm:text-lg' >
          ...
          <button type='submit' className={`h-10 w-full sm:w-96 mt-2 rounded-md text-black text-lg  ${darkMode ? 'bg-green-600 shadow-sm shadow-white/50' : 'bg-purple-600 font-semibold shadow-md shadow-purple-500'}`} >  Create Account</button>
        </form>

        <h1 className='text-center font-semibold '>Already have an Accout? <Link to={'/login'} className={`${darkMode ? 'text-green-400 font-bold' : 'text-purple-700 font-semibold text-lg'}`} >Log In</Link></h1>
      </div>
